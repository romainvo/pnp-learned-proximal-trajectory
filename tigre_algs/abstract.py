...
from tigre.utilities import gpu as gpu_utils
...
class IterativeReconAlg(object):
    ...
    def __init__(self, proj, geo, angles, niter, **kwargs):
        ...
        self.reproj_geo = copy.deepcopy(geo)
        self.niter = niter

        self.geo.check_geo(angles)

        self.intratomo = kwargs.pop('intratomo', False)
        if self.intratomo:
            print('****** INTRA-TOMO ********')
            self.reproj_angles = kwargs.pop('reproj_angles', None)
            self.reproj_geo.check_geo(self.reproj_angles)
        ...
        print('[CHECK] Prepare angles')
        if not all([hasattr(self, "angleindex"), hasattr(self, "angleblocks")]):
            self.set_angle_index()
        print('[CHECK] Prepare W')
        if not hasattr(self, "W"):
            self.set_w()
        print('[CHECK] Prepare V')
        if not hasattr(self, "V"):
            self.set_v()
        self.set_gpuids()
        
        print('[CHECK] Prepare res')
        ...
        print('[CHECK] Custom changes')
        ### CUSTOM CHANGES
        if hasattr(self, 'output_dir'):
            self.output_dir = path.Path(self.output_dir)
            (self.output_dir / 'imgs').mkdir(parents=True, exist_ok=True)
        if hasattr(self, 'dense_rc'):
            self.dense_metrics = np.zeros([1, niter])

        #### convergences checking
        self.slice_index = None
        if self.dataset_name == 'cork':
            self.slice_index = 300
            self.L_max = 0.1179 # max value on test set among slices located far from the axial edges
        elif self.dataset_name == 'walnut':
            self.slice_index = 250
            self.L_max = 0.502464 

        num_voxels = self.geo.nVoxel[0]
        self.convergence_indexes = np.linspace(start=num_voxels//5,
                                               stop=4*num_voxels//5,
                                               num=num_voxels//10, 
    ...
    def set_gpuids(self):
        ...
        print(self.gpuids)
        if self.pnp:
            self.gpuids.devices = [0]
            self.gpuids.name = gpu_names[0]
        
    def set_init_lr(self):
        # op_norm are pre-computed for each dataset
        if self.dataset_name == 'cork':
            op_norm = 52.74 if self.num_proj == 720 else 13.87 # num_proj=50
        elif self.dataset_name == 'walnut':
            op_norm = 30.55 # walnut-50p            
        if self.primal_dual:
            self.primal_lr = self.init_lr / op_norm
            self.dual_lr = 1 / op_norm
            self.dual_lr = self.dual_lr / self.init_lr

            print(f'Op-norm={op_norm:.2f}')
        else:
            self.primal_lr = self.init_lr / op_norm**2
    ...
    def set_res(self):
        ...
        if init == "FDK":
            self.res = FDK(self.proj, self.geo, self.angles, gpuids=self.gpuids)
    ...
    def error_measurement(self, res_prev, iter):
        ...
        if hasattr(self, 'dense_rc'):
            self.dense_metrics[:, iter] = np.mean((self.res - self.dense_rc)**2)
        if self.computel2:
    ...
    def geterrors(self):
        if self.computel2:
            if hasattr(self, 'dense_rc'):
                return np.concatenate((self.l2l, self.lq, self.dense_metrics), axis=0)
            else:
                return np.concatenate((self.l2l, self.lq), axis=0)
        else:
            if hasattr(self, 'dense_rc'):
                return np.concatenate((self.lq, self.dense_metrics), axis=0)
            else:
                return self.lq

    def norm(self, x):
        if self.dataset_name == 'cork':
            norm = np.linalg.norm(x[self.convergence_indexes][:, 256:256+512, 256:256+512].reshape(-1))
        elif self.dataset_name == 'walnut':
            norm = np.linalg.norm(x[self.convergence_indexes][:, 100:400, 100:400].reshape(-1))
    ...
    def residual_norm(self, a, b):        
        if self.dataset_name == 'cork':
            norm = np.linalg.norm((
                a[self.convergence_indexes][:, 256:256+512, 256:256+512] \
                - b[self.convergence_indexes][:, 256:256+512, 256:256+512]).reshape(-1)
            )
        elif self.dataset_name == 'walnut':
            norm = np.linalg.norm((
                a[self.convergence_indexes][:, 100:400, 100:400] \
                - b[self.convergence_indexes][:, 100:400, 100:400]).reshape(-1)
            )
    ...
    def convergence_check(self):
        ...
        for cvg_y_index in self.convergence_indexes:
            cvg_ref = self.dense_rc[cvg_y_index]
            cvg_output = self.res[cvg_y_index]
            if self.dataset_name == 'cork':
            ...
            ref_norm = np.linalg.norm(cvg_ref.reshape(-1))
            ref_range = np.max(cvg_ref) - np.min(cvg_ref)
            err_norm = np.linalg.norm(cvg_ref.reshape(-1) - cvg_output.reshape(-1))
            err_mse = np.mean((cvg_ref.reshape(-1) - cvg_output.reshape(-1))**2)

            snr = 10 * np.log10(ref_norm**2 / err_norm**2)
            snr_s.append(snr)

            mses_s.append(err_mse)

        mse = np.mean(mses_s)
        psnr = 10 * np.log10(self.L_max**2 / mse)
        snr = np.mean(snr_s)
...
def decorator(IterativeReconAlg, name=None, docstring=None):  # noqa: N803
    ...
    def iterativereconalg(proj, geo, angles, niter, **kwargs):
        ...
        # FIXME ? redondant with the call inside IterativeReconAlg constructor
