...
parser = argparse.ArgumentParser(description = "InDI test")
...
if __name__ == '__main__':
    ...
    ### setup InDI
    delta_t = 1 / train_args.num_timesteps
    timesteps = train_args.num_timesteps * np.linspace(delta_t, 1,
                            num=train_args.num_timesteps,
                            endpoint=True, 
                            dtype=np.float32)
    ## reverse timesteps, sparse=x(t=T), ref=x(t=0)
    timesteps = torch.from_numpy(np.ascontiguousarray(timesteps[::-1])).to(DEVICE)
            ...
            for batch_idx, (batch) in tqdm(enumerate(dataloader), total=len(dataloader), desc='Batch', position=1, leave=True):
                ...
                with torch.no_grad():
                    with torch.autocast(device_type=DEVICE, dtype=torch.float16, enabled=test_args.amp):
                        for t in tqdm(timesteps, total=100, position=2, leave=False):
                            output = model(input, 
                                        timesteps=t.repeat(input.shape[0]),
                                        residual_learning=train_args.residual_learning)
                                
                            output = (1 / t) * output.clamp(min=0) + (1 - 1 / t) * input
                            ...
                            if patch_size > dataset.num_voxels:
                                ...
                                input = torchvision.transforms.functional.pad(
                                    output, 
                                    padding=(padding_left, padding_left, padding_right, padding_right),
